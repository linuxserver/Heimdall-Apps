...
namespace App\SupportedApps\AdGuardHome;
    ...
    public function livestats()
    {
        ...
        if ($details) {
            ...
            if (is_array($details->dns_queries)) {
                $data["dns_queries"] = number_format(
                    array_sum($details->dns_queries)
                );
                $data["blocked_filtering"] = number_format(
                    array_sum($details->blocked_filtering)
                );
            } else {
                $data["dns_queries"] = number_format($details->dns_queries);
                $data["blocked_filtering"] = number_format(
                    $details->blocked_filtering
                );
            }
        }
