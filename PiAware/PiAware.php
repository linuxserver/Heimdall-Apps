...
namespace App\SupportedApps\PiAware;
...
class PiAware extends \App\SupportedApps implements \App\EnhancedApps
{
    ...
    public function livestats()
    {
        ...
        if ($details) {
            $filtered_total = array_filter($details["aircraft"], function (
                $element
            ) {
                return $element["seen"] <= 58;
            });
            $data["total"] = count($filtered_total);
            $filtered_positions = array_filter($filtered_total, function (
                $element
            ) {
                return array_key_exists("seen_pos", $element) &&
                    $element["seen_pos"] < 60;
            });
