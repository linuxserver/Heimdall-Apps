...
namespace App\SupportedApps\Jaeger;
...
class Jaeger extends \App\SupportedApps implements \App\EnhancedApps
{
    ...
    private function tracesUrl()
    {
        ...
        if (!empty($operation)) {
            $endpoint .= "&operation=" . urlencode($operation);
        }
        if (!empty($tags)) {
            $endpoint .= "&tags=" . urlencode($tags);
        }
        if (!empty($lookback)) {
            $lookbackFormatted = str_replace(
                "d",
                " days",
                str_replace("h", " hours", $lookback)
            );
            $timestampForOffset = time();
            $offset =
                $timestampForOffset -
                strtotime("-" . $lookbackFormatted, $timestampForOffset);
        ...
        if (!empty($minDuration)) {
            $endpoint .= "&minDuration=" . urlencode($minDuration);
        }
        if (!empty($maxDuration)) {
            $endpoint .= "&maxDuration=" . urlencode($maxDuration);
        }
        if (!empty($limit)) {
        ...
        if ($details) {
            ...
            if ($data["traces"] > 0) {
                ...
                $data["avg_duration"] = round(
                    array_sum($durations) / $data["traces"] / 1000,
                    1
                );
            }
        }
