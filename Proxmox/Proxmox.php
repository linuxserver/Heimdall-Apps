...
class Proxmox extends \App\SupportedApps implements \App\EnhancedApps {
    ...
    public function livestats()
    {
        ...
        if ($nodes == ['']) {
            $nodes = array_map(function ($v) {return $v->node;}, $this->apiCall('nodes'));
        }
        ...
        foreach($nodes as $node){
            ...
            $vm_running += count(array_filter($vm_stats, function($v){return $v->status == 'running';}));

            $container_stats = $this->apiCall('nodes/' . $node . '/lxc');
            $container_total += count($container_stats);
            $container_running += count(array_filter($container_stats, function($v){return $v->status == 'running';}));
        }
